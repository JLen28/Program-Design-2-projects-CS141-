...
//  UIN: 676805596
//  
//  I utilized Proffesor Reed's previous program 5 solution upon his authorization.
// Instructions:
...
//---------------------------------------------------------------------------------------
void preAppend(Node *&pHead,int board[],int squaresPerSide,int moveNumber,int score)
{
    Node *pTemp = new Node;
    pTemp->moveNumber = moveNumber;
    pTemp->score = score;
    for(int i = 0; i < squaresPerSide * squaresPerSide; i++) { pTemp->board[i] = board[i];}
    pTemp->pNext = pHead;
    pHead = pTemp;   
}
//--------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------
// here is where the function for undo needs to go. should accept the board and maybe squaresPerSide as well as moveNumber and score...
// ------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------------
// goal is to get this function to go back one Node eventually reaching one where there is no more undos...
int main()
{	
	...
	while (window.isOpen())
	{
       ...
       for(int j = 0; j< squaresPerSide; j++){
		...
		char name[ 81];
        int e = j * squaresPerSide + i;
		// Squares with a 0 value should not have a number displayed
		if( board[ e] == 0) {
			strcpy( name, "");   // "print" a blank text string
		}
		else {
		   sprintf( name, "%d", board[ e]);   // "print" the square creation order number into a string to be stored in the square
		}
		// Set each array pointer have the address of a new Square, created with a Square constructor
		squaresArray[e] = Square(50, 50 * i + i*10,j * 50 + 10 * j, sf::Color(0,100,50), true, name);
		// Draw the square
		window.draw( squaresArray[ e].getTheSquare());
		// Draw the text associated with the Square, in the window with the indicated color and text size
		int red = 255, green = 255, blue = 255;
		squaresArray[e].displayText( &window, font, sf::Color(red, green, blue), 30);
	}
       }
        ...
        switch (userInput) {
           ...
           // ... handle other user inputs ...
            ...
            case 'u':
                    if(moveNumber == 1){
                      std::cout << "*** You cannot undo past the beginning of the game.  Please retry. ***" << std::endl; // user tried to undo but its the beginning of the game..
                      continue;
                    }
                    std::cout << "* Undoing move *" << std::endl;               // user is undoing a move.
                    undo(pHead,board,squaresPerSide,moveNumber,score);
                    continue;
    ...
    if( boardChangedThisTurn( previousBoard, board, squaresPerSide)) {
            ...
            placeRandomPiece( board, squaresPerSide);
        // if board was a valid change, then a node must be created for the linked list..
        // 
        // 
        
            moveNumber++;
            // Update move number after a valid move
            preAppend(pHead,board,squaresPerSide,moveNumber,score);
        }
